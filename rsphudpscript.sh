...
show_usage_and_exit() {
	echo
	echo -e "\t$(tbold)$SCRIPT_NAME$(treset) - RSPH-UDP server install script"
	echo
	echo -e "Usage:"
	echo
	echo -e "$(tbold)Install RSPH-UDP$(treset)"
	...
	echo -e "\t-l, --local <file>\tInstall specified RSPH-UDP binary instead of download it."
	echo -e "\t--version <version>\tInstall specified version instead of the latest."
	echo
	echo -e "$(tbold)Remove RSPH-UDP$(treset)"
	echo -e "\t$0 --remove"
	echo
		...
		_is_frash_install=1
						...
						if [[ -n "$_is_frash_install" ]]; then
							...
							echo
								...
								start_services
								echo
								echo -e "$(tbold)RSPH-UDP has been Successfully Installed.$VERSION.$(treset)"
								echo
								fi
}
...
perform_remove() {
	...
	echo
	echo -e "$(tbold)Congratulation! RSPH-UDP has been successfully removed from your server.$(treset)"
	echo
